...
type ShoppingList = { id: string; items: ListItem[]; completedAt?: string };
...
const initialState = (): AppState => {
  ...
  return { version: 1, categories, products, active: { id: uid(), items: [] }, template: { id: uid(), items: [] }, history: [], frequency: {} };
};
...
const displayDetail = (item: ListItem) => [item.quantity && `${item.quantity}${item.unit ? ` ${item.unit}` : ""}`, item.note].filter(Boolean).join(" · ");

function Thumb({ label, large = false }: { label: string; large?: boolean }) { return <span className={`thumb ${large ? "thumb-large" : ""}`} aria-hidden="true">{label.slice(0, 1).toUpperCase()}</span>; }
...
export default function Home() {
  ...
  const saveItem = () => { if (!editing) return; updateList({ ...list, items: list.items.map((i) => i.id === editing.id ? { ...i, ...draft } : i) }); setSheet(null); };
  const createProduct = () => {
    if (!categoryId || !draft.name.trim()) return;
    const product = { id: uid(), categoryId, name: draft.name.trim(), builtIn: false };
    setState((s) => ({ ...s, products: [...s.products, product] }));
    const cat = state.categories.find((c) => c.id === categoryId)!;
    const item = { id: uid(), productId: product.id, categoryId, categoryName: cat.name, name: product.name, quantity: draft.quantity, unit: draft.unit, note: draft.note, bought: false };
  ...
  const finish = () => { setState((s) => ({ ...s, history: [{ ...s.active, id: uid(), completedAt: new Date().toISOString(), items: s.active.items.map((i) => ({ ...i })) }, ...s.history].slice(0, 2), active: { id: uid(), items: [] } })); setSheet(null); setTab("new"); };
  ...
  const openEdit = (item: ListItem) => { setEditing(item); setDraft({ name: item.name, quantity: item.quantity, unit: item.unit || "шт.", note: item.note }); setSheet("item"); };
  const openNewProduct = () => { setDraft({ name: "", quantity: "", unit: "шт.", note: "" }); setSheet("product"); };
